...
if ($userId <= 0) {
    $requestUri = (string)($_SERVER['REQUEST_URI'] ?? '/my/proxies');
    $requestPath = (string)(parse_url($requestUri, PHP_URL_PATH) ?? '/my/proxies');
    if (!str_starts_with($requestPath, '/') || str_starts_with($requestPath, '//')) {
        $requestUri = '/my/proxies';
    }
...
foreach ($services as $service) {
    ...
    $actionsHtml = '<div class="d-flex flex-wrap gap-1">'
        . '<a class="pm-pill-btn is-active" href="/proxy/manage?service_id=' . mp_h(rawurlencode($serviceId)) . '">Manage</a>';
    $actionsHtml .= '</div>';
    ...
    $tableRows[] = [
        ...
        'traffic' => mp_s($service['traffic_remains'] ?? '-'),
...
<main class="container my-4 sog-ui">
    <style>
        #my_proxies_grid__tbl_wrapper .dt-buttons .btn-excel,
        #my_proxies_grid__tbl_wrapper .dt-buttons .btn-print {
    ...
    <?php endif; ?>
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    ...
                    <div>
                        <a class="btn btn-primary" href="/all_proxy">Buy more</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <?php else: ?>
            <?php
            $columns = ['actions_html', 'title', 'host', 'port', 'login', 'password', 'public_ip', 'country', 'status', 'expires_at', 'traffic'];
            $headers = [
                'title' => 'Title',
                'host' => 'Host',
                'port' => 'Port',
