...
final class APIInfaticaIo_residential
{
    ...
    {
        $this->api->set_residential_api_key($api_key);
        $this->api->set_api_key($api_key);
    }

    private function with_residential_api_key(callable $callback): mixed
    {
        $previous = $this->api->api_key;
        $residentialKey = trim($this->api->api_key_residential);
        if ($residentialKey !== '') {
            $this->api->set_api_key($residentialKey);
        }
        try {
            return $callback();
        } finally {
    ...
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->stats());
    }

    /** @return array<mixed>|null */
    public function online_statistics(string $country = '', string $period = '', int $interval = 1): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->online_info($country, '', $period, $interval));
    }

    /** @return array<mixed>|null */
    public function geos(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->nodes_info());
    }

    /** @return array<mixed>|null */
    public function detailed_geos(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->count_by_geo());
    }

    /** @return array<mixed>|null */
    public function ipv6_detailed_geos(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->count_by_geo_v6());
    }

    /** @return array<mixed>|null */
    public function keys(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->keys());
    }

    /** @return array<mixed>|null */
    public function subdivision_codes(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->subdivision_codes());
    }

    /** @return array<mixed>|null */
    public function isp_codes(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->isp_codes());
    }

    /** @return array<mixed>|null */
    public function zip_codes(string $country = ''): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->zip_codes($country));
    }

    /** @return array<mixed>|null */
    public function geo_db(): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->geo());
    }
    ...
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_packages_filtered($packages));
    }

    /** @return array<mixed>|null */
    public function package_info(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_info($package_key));
    }

    /** @return array<mixed>|null */
    public function package_usage(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_usage($package_key));
    }

    /** @return array<mixed>|null */
    public function usage(bool $post = false): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->usage($post));
    }

    /** @return array<mixed>|null */
    public function usage_pagination(int $page = 1): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->usage_pagination($page));
    }

    /** @return array<mixed>|null */
    public function traffic_details(string $package_key, string $period = 'daily'): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->traffic_details($package_key, $period));
    }

    /** @return array<mixed>|null */
    public function create_package_bytes(int $limit_bytes, string $expired_at): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_create([
    ...
    public function set_traffic_limit_bytes(string $package_key, int $limit_bytes, string $expired_at = ''): ?array
    {
        ...
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_update($package_key, $form));
    }

    /** @return array<mixed>|null */
    public function prolongate(string $package_key, string $expired_at): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_prolongate($package_key, $expired_at));
    }
    ...
    /** @return array<mixed>|null */
    public function generate_access(string $package_key, string $name, string $login, string $password, string $country = '', int $rotation = 3600, int $format = 3): ?array
    {
        $form = [
        ...
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_generate($package_key, $form));
    }

    /** @return array<mixed>|null */
    public function generate_access_from_options(string $package_key, array $options): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_generate_from_options($package_key, $options));
    }

    /** @return array<mixed>|null */
    public function api_tool_access(string $package_key, array $options): ?array
    {
        return $this->with_residential_api_key(function () use ($package_key, $options): ?array {
    ...
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_updatelist_from_options($package_key, $id, $name, $options));
    }

    /** @return array<mixed>|null */
    public function regenerate_access_password(string $package_key, int|string $id, string $name): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_pwd_regenerate($package_key, $id, $name));
    }

    /** @return array<mixed>|null */
    public function view_access(string $package_key, int|string $id, string $name): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_viewlist($package_key, $id, $name));
    }

    /** @return array<mixed>|null */
    public function remove_access(string $package_key, int|string $id, string $name): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_removelist($package_key, $id, $name));
    }

    /** @return array<mixed>|null */
    public function lists(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->package_lists($package_key));
    }

    /** @return array<mixed>|null */
    public function suspend(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_suspend($package_key));
    }

    /** @return array<mixed>|null */
    public function resume(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_resume($package_key));
    }

    /** @return array<mixed>|null */
    public function deactivate(string $package_key): ?array
    {
        return $this->with_residential_api_key(fn(): ?array => $this->api->reseller_package_deactivate($package_key));
    }

    public function check_ip_block(string $ip): ?string
    {
        return $this->with_residential_api_key(fn(): ?string => $this->api->check_ip_block($ip));
    }
