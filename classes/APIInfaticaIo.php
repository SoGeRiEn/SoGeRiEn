...
final class APIInfaticaIo
{
    ...
    {
        if (method_exists($this->Transport, $name)) {
            return $this->Transport->{$name}(...$arguments);
        }

        throw new BadMethodCallException('Unknown Infatica method: ' . $name);
    }

    public function __get(string $name): mixed
    {
        if (property_exists($this->Transport, $name)) {
            return $this->Transport->{$name};
        }

        trigger_error('Undefined Infatica property: ' . $name, E_USER_NOTICE);
        return null;
    }
