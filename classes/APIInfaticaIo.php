...
final class APIInfaticaIo
{
    ...
    /** @return array<mixed>|null */
    public function client_geo_nodes(bool $mobile = false, bool $dc = false): ?array
    {
        if (Sogerien::$debag) { Sogerien::Debager()->log_input(__CLASS__, __FUNCTION__, func_get_args()); }
        $flags = $this->build_client_proxy_type_flags($mobile, $dc);
        if ($flags === null) {
    ...
    private function collect_proxy_list_types(string $type): array
    {
        ...
        if ($type === '' || $type === 'all') {
            return Sogerien::Debager()->capture_return(['residential', 'mobile', 'isp'], __CLASS__, __FUNCTION__);
        }
    ...
    {
        if (Sogerien::$debag) { Sogerien::Debager()->log_input(__CLASS__, __FUNCTION__, func_get_args()); }
        $margin = $this->resolve_pricing_margin();
        $days = 364;
        $result = [];
        ...
        foreach ($rows as $row) {
            ...
            foreach ($tiers as $volume => $base_price) {
                ...
                $unit_price = round(((float)$base_price) * $margin, 2);
                $total_price = round($unit_price * (float)$volume_int, 2);
                ...
                $new_row['price_per_gb'] = number_format($unit_price, 2, '.', '');
    ...
    private function resolve_pricing_margin(): float
    {
        if (Sogerien::$debag) { Sogerien::Debager()->log_input(__CLASS__, __FUNCTION__, func_get_args()); }
        $margin = (float)($this->pricing['marginPercent'] ?? 1.5);
        if ($margin <= 0.0) {
            $margin = 1.5;
        }
        return Sogerien::Debager()->capture_return($margin, __CLASS__, __FUNCTION__);
    }
    ...
    private function resolve_pricing_tiers_by_category(string $category): array
    {
        ...
        $tiers = [];
        if (isset($this->pricing[$category]) && is_array($this->pricing[$category])) {
            foreach ($this->pricing[$category] as $volume => $price) {
    ...
    public function package_generate(string $package_key, array $form): ?array
    {
        ...
        $package_key = $this->normalize_package_key($package_key);
        if ($package_key === null) return Sogerien::Debager()->capture_return(null, __CLASS__, __FUNCTION__);
        if (trim((string)($form['proxy-list-name'] ?? '')) === '' || trim((string)($form['proxy-list-login'] ?? '')) === '') {
            $this->fail('proxy-list-name and proxy-list-login are required');
            return Sogerien::Debager()->capture_return(null, __CLASS__, __FUNCTION__);
        }
    ...
    private function normalize_proxy_api_type(string $proxy_type): string
    {
        ...
        if ($proxy_type === 'residential' || $proxy_type === 'mobile' || $proxy_type === 'isp') {
            return Sogerien::Debager()->capture_return($proxy_type, __CLASS__, __FUNCTION__);
        }
        if ($proxy_type === 'dc' || $proxy_type === 'datacenter') {
            return Sogerien::Debager()->capture_return('isp', __CLASS__, __FUNCTION__);
        }
        $this->fail('proxy type must be residential|mobile|isp');
        return Sogerien::Debager()->capture_return('', __CLASS__, __FUNCTION__);
    }
     ...
     * @return array<string,string>|null
     */
    private function build_client_proxy_type_flags(bool $mobile, bool $dc): ?array
    {
        if (Sogerien::$debag) { Sogerien::Debager()->log_input(__CLASS__, __FUNCTION__, func_get_args()); }
        if ($mobile && $dc) {
            $this->fail('mobile and dc cannot be true together');
            return Sogerien::Debager()->capture_return(null, __CLASS__, __FUNCTION__);
        }
        if ($mobile) {
